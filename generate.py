...
life history, and character that might impact his voice. He can be from anywhere in the world, the UK, France, \
Sweden, Germany, Australia, Ireland, the US etc and any accent. Also consider his mood which could be anything \
from happy, sad, angry, excited, afraid etc. The man currently lives in rural Florida. Consider many different \
aspects of pacing, speed, volume and punctuation.""")
...
async def generate_multiple_voice_instructions():
    tasks = [gen_voice_instructions(
        system_prompt=voice_guidelines_system_prompt.format(), 
        voice_prompt=gen_man_voice_prompt.format(), 
...
voice_dataset = weave.Dataset(
    name=WEAVE_DATASET_NAME,
    rows=voice_data_list
)
